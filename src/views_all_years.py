...
def get_one_lang_views_by_titles(langcode, titles):
    # ---
    all_data = {}
    # ---
    for i in range(0, len(titles), 500):
        ...
        # ---
        data = article_views_all_years(langcode, group)
        # ---
...
def load_one_lang_views(langcode, titles) -> dict[str, dict]:
    """
    Load view statistics for one language and a list of titles for all years.
    """
    # ---
    data = {}
    # ---
    if "zero" in sys.argv:
        ...
        data = get_one_lang_views_by_titles(langcode, titles)
    # ---
    return data
...
def get_one_lang_views(langcode, titles, json_file, maxv=0) -> dict:
    ...
    if maxv > 0 and len(titles_not_in_file) > maxv:
        ...
        views_t = load_one_lang_views(langcode, titles_not_in_file)
